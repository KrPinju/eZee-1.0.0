...
const SIDEBAR_WIDTH = "17rem" // Was 16rem
...
>(
  (
    ...
    if (isMobile) {
       // Use icon collapsible behavior for mobile trigger (offcanvas sheet)
       return (
     ...
     // Desktop: Use 'icon' collapsible behavior by default
     return (
       <div
         ref={ref}
         className={cn(
         ...
         data-state={state}
         // Set data-collapsible based on the state for styling
         data-collapsible={state === "collapsed" ? collapsible : ""}
         data-variant={variant}
         data-side={side}
       >
         {/* This div handles the space the sidebar occupies, pushing content */}
         <div
           className={cn(
             "relative h-svh bg-transparent transition-[width] duration-300 ease-in-out",
             state === 'expanded' ? "w-[--sidebar-width]" : "w-[--sidebar-width-icon]",
              // Adjust padding if floating/inset, otherwise use full icon width
         ...
         <div
           className={cn(
             ...
             variant === "floating" || variant === "inset"
               ? (state === 'collapsed' ? "p-2 w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]" : "p-2 w-[--sidebar-width]")
               : (state === 'collapsed' ? "w-[--sidebar-width-icon]" : "w-[--sidebar-width]"),
             // Apply border only for sidebar variant
           ...
           <div
             data-sidebar="sidebar"
              className={cn("flex h-full w-full flex-col bg-sidebar",
                variant === "floating" && "rounded-lg border border-sidebar-border shadow" // Apply floating styles only if floating
              )}
            >
             {children}
           </div>
...
// Rail is removed as the button is now in the header

const SidebarInset = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<"main">
>(({ className, ...props }, ref) => {
   const { state, isMobile } = useSidebar(); // Use hook to get state
    ...
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
         state === 'collapsed' ? "-mt-8 opacity-0" : "opacity-100", // Use state for conditional styling
        className
      )}
      {...props}
    />
    ...
    <Comp
      ...
      className={cn(
        ...
        "after:absolute after:-inset-2 after:md:hidden",
         state === 'collapsed' ? 'hidden' : 'flex', // Use state
        className
      )}
      {...props}
    />
   ...
   "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-primary data-[active=true]:font-medium data-[active=true]:text-sidebar-primary-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
   {
     variants: {
       ...
       size: {
         default: "h-8 text-sm",
         sm: "h-7 text-xs",
         lg: "h-12 text-sm group-data-[collapsible=icon]:!p-0", // Adjusted lg size for icon state
       },
     },
     defaultVariants: {
       variant: "default",
  ...
  (
    ...
    const button = (
      <Comp
        ...
        // Apply variants and ensure collapsed state styles
        className={cn(
    ...
    if (typeof tooltip === "string") {
      tooltip = {
        children: tooltip,
        className: tooltip.className || "bg-primary text-primary-foreground", // Default tooltip style
      }
    }

    return (
      <Tooltip>
         {/* TooltipTrigger only wraps the button when needed */}
...
export {
  ...
  // SidebarRail, // Removed
  SidebarSeparator,
  SidebarTrigger, // Keep mobile trigger if needed, but primary toggle is now in header
  useSidebar,
}
